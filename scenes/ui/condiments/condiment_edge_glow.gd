class_name CondimentEdgeGlow
extends TextureRect

## Screen-edge veil that flashes when a belt condiment is drunk.
...
func _on_consume_started(_slot_index: int, condiment: Condiment) -> void:
	if GameManager.skip_presentation or condiment == null:
		return
	_play(_to_glow_color(condiment.liquid_color))
...
func _to_glow_color(liquid: Color) -> Color:
	# Keep the condiment hue. Nudge value up so the rim still reads on a bright map.
