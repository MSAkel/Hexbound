...
# One row in the run-info panel for a single map segment's turn score and gold.

var segment_index: int = -1

@onready var segment_score: Label = $SegmentScore
@onready var segment_gold: Label = $GoldContainer/SegmentGold
...
func _on_segment_turn_results_changed(changed_index: int, score: int, gold: int) -> void:
...
func _refresh(score: int, gold: int) -> void:
