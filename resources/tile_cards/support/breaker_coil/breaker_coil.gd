...
func _on_activate_tile_card(tile: Hex) -> void:
	if tile.map.get_segment_breaks(_get_segment_index(tile)) <= 0:
		...
		return
	var next_producers := _get_next_tile_cards_in_trigger_order(tile, 1, TileCardType.PRODUCER)
...
func get_trigger_preview_coords(hover_tile: Hex) -> Array[Vector2i]:
	return _coords_for_placed_tile_cards(
		hover_tile,
		_get_next_tile_cards_in_trigger_order(hover_tile, 1, TileCardType.PRODUCER)
	)
