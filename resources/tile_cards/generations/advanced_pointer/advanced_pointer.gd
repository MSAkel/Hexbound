...
## +14 Energy per Following adjacent Energy card. +20 extra if 2 or more.
...
func _get_pointer_amount(tile: Hex) -> float:
	var neighboring_energy := _get_following_adjacent_tile_cards_by_product(tile, Product.SCORE)
