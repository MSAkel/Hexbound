extends TileCard
## +20 Flavour. If the previous card in fire order is a Kitchenware, fire again this card once.
...
func _on_activate_tile_card(tile: Hex) -> void:
	...
	if previous.is_empty() or previous[0].type != TileCardType.KITCHENWARE:
		return
