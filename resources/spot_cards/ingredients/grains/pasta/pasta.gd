extends TileCard
## +10 Flavour, plus +5 per consecutive Flavour Ingredient immediately before this card.

const STREAK_BONUS := 5


func _on_activate_tile_card(tile: Hex) -> void:
	_pay_effect_amount(tile)


func _effect_amount(tile: Hex) -> float:
	if tile == null:
		return _get_production_amount()
	var streak := _effect_preview_cards(tile).size()
	return _get_production_amount() + float(STREAK_BONUS * streak)


func _effect_preview_cards(tile: Hex) -> Array[TileCard]:
	return _get_consecutive_previous_energy_producers(tile)


func _get_consecutive_previous_energy_producers(tile: Hex) -> Array[TileCard]:
	var streak: Array[TileCard] = []
	var hexes := tile.map.get_hexes_in_trigger_order()
	var self_index := tile.map.get_hex_trigger_order_index(tile)
	if self_index < 0:
		return streak
	# Walk the previous hexes. Empty tiles and non-Energy cards break the run.
	for i in range(self_index - 1, -1, -1):
		var card := hexes[i].active_tile_card
		if card == null:
			break
		if card.type != TileCardType.INGREDIENT:
			break
		streak.append(card)
	return streak
