...
func _on_activate_rune(tile: Hex) -> void:
	var prod_runes: Array[Rune] = get_all_placed_runes(tile, Rune.RuneType.PRODUCER)
	...
	for rune in prod_runes:
		...
		if unempowered_runes.is_empty():
			create_floating_text(tile, "No unempowered runes")
			break
		var target: Rune = unempowered_runes.pick_random()
		target.empower()
		unempowered_runes.erase(target)
		create_floating_text(tile, "Empowered %s" % target.name)
